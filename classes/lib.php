...
class lib{
    ...
    public static function get_tickets_by_user($userid){
        ...
        $ticket = $DB->get_record('local_tickets', array('created_by' => $userid));
    ...
    public static function get_own_tickets(){
        global $DB, $USER;
        $tickets = array_values($DB->get_records('local_tickets', array('created_by' => $USER->id), 'created_at DESC'));
        $tickets = self::format_tickets_date($tickets);
        return $tickets;
    }
    ...
    public static function get_ticket($ticketid){
        ...
        $ticket = $DB->get_record('local_tickets', array('id' => $ticketid));
        $ticket = self::format_tickets_date($ticket);
        return $ticket;
    }
    ...
    /**
     * Get tickets count with conditions.
     * @return integer on success.
     */
    ...
    public static function get_own_tickets_count(){
        global $DB, $USER;
        $count = $DB->count_records('local_tickets', array('created_by' => $USER->id));
        return $count;
    }
    ...
    public static function delete_ticket($ticketid){
        ...
        global $DB;
        $success = $DB->delete_records('local_tickets', array('id' => $ticketid));
        return $success;
    }
    ...
    public static function submit_ticket($formdata){
        ...
        $record->updated_at = time();
        $record->updated_by = $USER->id; //meow
    ...
    private static function get_formatted_comments($comments){
        ...
        for($i = 0; $i < count($comments); $i++){
            if(in_array($comments[$i]->created_by, $users_ids)){
                continue;
            }
            $users_ids[] = $comments[$i]->created_by;
        }
        $users = $DB->get_records_list('user', 'id', $users_ids, '','id,firstname,lastname,username,picture,imagealt');
